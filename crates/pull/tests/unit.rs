...
// ───────────────────────── the dated session regime ─────────────────────────

/// Three different closing times now coexist on one afternoon.
///
/// This is the test the whole module exists for. Before 2026-08-03 a single
/// constant served every segment; from that date it serves none of them
/// correctly, and the segment this engine *sweeps* has the shortest session of
/// the three rather than the longest.
#[test]
fn three_closes_coexist_from_the_third_of_august() {
    use pull::regime::{Segment, session_at};
    use pull::session::Day;

    let day = Day::new(2026, 8, 3).expect("a real date");

    let index = session_at(Segment::SpotIndex, day).expect("verified");
    let cash = session_at(Segment::CashNonDerivative, day).expect("verified");
    let fno = session_at(Segment::EquityDerivative, day).expect("verified");

    assert_eq!(
        index.close,
        Some(915),
        "15:15 — the index freezes with its constituents"
    );
    assert_eq!(
        cash.close,
        Some(930),
        "15:30 — a share with no derivative is not CAS eligible"
    );
    assert_eq!(
        fno.close,
        Some(940),
        "15:40 — ten minutes to react to the auction"
    );

    assert_eq!(index.bars(), Some(360));
    assert_eq!(cash.bars(), Some(375));
    assert_eq!(fno.bars(), Some(385));

    assert!(
        index.close < cash.close && cash.close < fno.close,
        "the swept instrument closes EARLIEST, which is the counter-intuitive \
         part and the reason a filter written for the derivatives clock would \
         admit 25 minutes of bars the index never printed"
    );
}

/// The boundary, on both sides, for the segment this engine sweeps.
#[test]
fn the_swept_index_changes_clock_on_exactly_the_third_of_august() {
    use pull::regime::{Segment, session_at};
    use pull::session::Day;

    let before =
        session_at(Segment::SpotIndex, Day::new(2026, 8, 2).expect("real")).expect("verified");
    let on = session_at(Segment::SpotIndex, Day::new(2026, 8, 3).expect("real")).expect("verified");

    assert_eq!(before.bars(), Some(375), "the day before is the old regime");
    assert_eq!(on.bars(), Some(360), "the day itself is the new one");
    assert_ne!(before.close, on.close, "the boundary is real, not cosmetic");
}

/// A date with no retrieved circular is REFUSED, never served from a neighbour.
///
/// Serving the nearest known row would apply a different era's clock and
/// mis-window every bar of that day — and a mis-windowed bar is simply absent,
/// which is indistinguishable from a quiet market.
#[test]
fn an_era_with_no_circular_is_refused_by_name() {
    use pull::regime::{RegimeError, Segment, session_at};
    use pull::session::Day;

    let old = Day::new(2005, 6, 15).expect("a real date");
    let why = session_at(Segment::SpotIndex, old).expect_err("no circular for 2005");

    // `RegimeError` is `#[non_exhaustive]`, so a `let` binding is refutable
    // even with one variant today — the attribute is the promise that a second
    // one may arrive, and this match is what keeps that promise cheap.
    let RegimeError::Unverified {
        segment,
        day,
        verified_from,
        source,
        ...
        panic!("expected an Unverified refusal, got {why:?}")
    };
    assert_eq!(segment, Segment::SpotIndex);
    assert_eq!(day, old);
    assert_eq!(
        verified_from,
        Some(Day::new(2010, 1, 4).expect("real")),
        "the refusal says where the record does begin"
    );
    assert!(
        source.contains("UNVERIFIED"),
        "the row carries its own citation gap — got {source:?}"
    );

    let text = why.to_string();
    assert!(
        text.contains("2005-06-15") && text.contains("2010-01-04"),
        "the message names the date asked about and the first verified one — got {text:?}"
    );
}

/// Every row carries a citation, verified or not. A blank source is a row
/// nobody can check.
#[test]
fn every_regime_row_carries_its_source() {
    use pull::regime::{Segment, table};
    ...
    for segment in Segment::ALL {
        for row in table(segment) {
            ...
            assert_eq!(
                row.open.is_some(),
                row.close.is_some(),
                "{:?} row from {} is half-known, which is worse than unknown",
                segment,
                row.from
            );
            if !row.verified() {
                assert!(
                    row.source.contains("UNVERIFIED"),
                    "a refusal row must say so in its source — {:?} from {}",
                    segment,
                    row.from
                );
                assert_eq!(row.bars(), None, "a refusal row cannot derive a bar count");
            }
        }
    }
    assert_eq!(rows, 10, "4 index + 3 cash + 3 derivative rows");
}

/// The lookup cost does not depend on which row wins.
///
/// The table is a `const` array whose length is fixed at compile time, so the
/// walk is O(1) by construction. This asserts the flatness as a number anyway,
/// because `docs/07-o1-architecture.md` says a bound is built when a test
/// asserts it, not when the code looks right.
#[test]
fn the_regime_walk_is_flat_across_every_row() {
    use pull::regime::{Segment, session_at};
    use pull::session::Day;
    use std::time::Instant;

    const REPS: u32 = 200_000;
    let first = Day::new(2010, 1, 4).expect("real");
    let last = Day::new(2026, 8, 3).expect("real");

    let cost = |day| {
        let t = Instant::now();
        for _ in 0..REPS {
            let _ = std::hint::black_box(session_at(Segment::SpotIndex, std::hint::black_box(day)));
        }
        t.elapsed().as_nanos().max(1)
    };

    let (a, b) = (cost(first), cost(last));
    let (lo, hi) = (a.min(b), a.max(b));
    let ratio_x100 = hi * 100 / lo;
    println!(
        "regime walk: first row {a} ns, last row {b} ns over {REPS} reps · ratio {ratio_x100}/100"
    );
    assert!(
        ratio_x100 <= 400,
        "the last row must not cost meaningfully more than the first — \
         got {ratio_x100}/100. A regression here means someone replaced the \
         const-array walk with something that searches."
    );
}
